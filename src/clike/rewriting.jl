...
function index_expression(m, expr, args, arg_types)
    try
        isempty(types) && error("Indexing into nothing with func: $m")
        T = arg_types[1] # type to index into
        indices, index_types = args[2:end], arg_types[2:end]
...
function emit_constructor(m::LazyMethod, realtype, args)
    typed_expr(realtype, :curly, m, args...)
end

function rewrite_function{F}(m::LazyMethod, f::F, types::ANY, expr)
    # first give the backends a chance to do backend specific rewriting
    expr, rewritten = rewrite_backend_specific(m, f, types, expr)
    rewritten && return expr
    # if nothing got rewritten, do the backend independant rewrites:
    if f in (getfield, getindex, setindex!)
        index_expression(m, expr, args, arg_types)
